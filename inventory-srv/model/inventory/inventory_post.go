...
func (s *service) Sell(bookId int64, userId int64) (id int64, err error) {
	...
	if err != nil {
		log.Infof("[Sell] 事务开启失败", err.Error())
		return
	}
	defer func() {
		if err != nil {
	...
	deductInv = func() (errIn error) {
		...
		if errIn != nil {
			log.Infof("[Sell] 查询数据失败，err：%s", errIn)
			return errIn
		}

		if inv.Stock < 1 {
			errIn = fmt.Errorf("[Sell] 库存不足")
			log.Infof(errIn.Error())
			return errIn
		}

		r, errIn := tx.Exec(updateSQL, inv.Stock-1, inv.Version+1, bookId, inv.Version)
		if errIn != nil {
			log.Infof("[Sell] 更新库存数据失败，err：%s", errIn)
			return
		}

		if affected, _ := r.RowsAffected(); affected == 0 {
			log.Infof("[Sell] 更新库存数据失败，版本号%d过期，即将重试", inv.Version)
			// 重试，直到没有库存
			deductInv()
		}
	...
	if err != nil {
		log.Infof("[Sell] 新增销存记录失败，err：%s", err)
		return
	}
