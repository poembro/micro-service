...
func (s *service) PayOrder(orderId int64) (err error) {
	...
	if err != nil {
		log.Infof("[PayOrder] 查询 订单信息失败，orderId：%d, %s", orderId, err)
		return
	}
	...
	if orderRsp.Order.State == common.InventoryHistoryStateOut {
		...
		log.Infof("[PayOrder] 查询 订单已支付，orderId：%d", orderId)
		return
	}
	...
	if err != nil {
		log.Infof("[PayOrder] 事务开启失败", err.Error())
		return
	}
	defer func() {
		if err != nil {
	...
	if err != nil {
		log.Infof("[New] 新增支付单失败，%v, err：%s", orderRsp.Order, err)
		return
	}
