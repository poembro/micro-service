...
func main() {
    test := `[{"id":7,"pid":1,"name":"g"},{"id":1,"pid":0,"name":"a"},{"id":2,"pid":1,"name":"b"},{"id":3,"pid":1,"name":"c"},{"id":4,"pid":2,"name":"d"},{"id":5,"pid":3,"name":"e"},{"id":6,"pid":5,"name":"f"}]`
...
func buildTree(parentId int, data map[int]map[int]*TreeNode) []*TreeNode {
    ...
    for id, item := range data[parentId] {
		fmt.Println(" 第 %d 个元素追加到 %d 个元素的list里面 ", id, parentId)
        if data[id] != nil {
